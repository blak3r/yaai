...
<TR>
    ...
    <td nowrap width="10%" class="dataLabel">{$MOD.LBL_ASTERISK_YAAI_DEV}
      {if !empty($MOD.LBL_ASTERISK_YAAI_DEV_DESC)}
          [<a href="#" data-toggle="tooltip" title="{$MOD.LBL_ASTERISK_YAAI_DEV_DESC}">?</a>]:
      {/if}
    </td>
    <td width="25%" class="dataField">
    {if empty($config.asterisk_yaai_dev )}
        {assign var='asterisk_yaai_dev' value=$asterisk_config.asterisk_yaai_dev}
        {if $asterisk_config.asterisk_yaai_dev }
            {assign var='isChecked' value='checked'}
        {/if}
    {else}
        {assign var='asterisk_yaai_dev' value=$config.asterisk_yaai_dev}
        {if $config.asterisk_yaai_dev }
            {assign var='isChecked' value='checked'}
        {/if}
    {/if}
    <input type='hidden' name='asterisk_yaai_dev' value='0'>
    <input type='checkbox' name='asterisk_yaai_dev' size="45" value='1' {$isChecked} >
    </td>

    <td nowrap width="10%" class="dataLabel">{$MOD.LBL_ASTERISK_YAAI_DEBUG}
      {if !empty($MOD.LBL_ASTERISK_YAAI_DEBUG_DESC)}
          [<a href="#" data-toggle="tooltip" title="{$MOD.LBL_ASTERISK_YAAI_DEBUG_DESC}">?</a>]:
      {/if}
    </td>
    <td width="25%" class="dataField">
    {if empty($config.asterisk_yaai_debug )}
        {assign var='asterisk_yaai_debug' value=$asterisk_config.asterisk_yaai_debug}
        {if $asterisk_config.asterisk_yaai_debug }
            {assign var='isChecked' value='checked'}
        {/if}
    {else}
        {assign var='asterisk_yaai_debug' value=$config.asterisk_yaai_debug}
        {if $config.asterisk_yaai_debug }
            {assign var='isChecked' value='checked'}
        {/if}
    {/if}
    <input type='hidden' name='asterisk_yaai_debug' value='0'>
    <input type='checkbox' name='asterisk_yaai_debug' size="45" value='1' {$isChecked} >
    </td>
