...
elseif (!isset($_POST['record']) && !is_admin($current_user)) sugar_die ("Unauthorized access to user administration.");
...
// copy the group or portal user name over.  We renamed the field in order to ensure auto-complete would not change the value
    ...
    // <abcona>
    // Extensions for Sugar-Asterisk Intgeration
    //
    if (isset($_REQUEST['asterisk_inbound_c']))
    {
       $focus->asterisk_inbound_c = 1;
    }
    else
    {
        $focus->asterisk_inbound_c = 0;
    }
    if (isset($_REQUEST['asterisk_outbound_c']))
    {
       $focus->asterisk_outbound_c = 1;
    }
    else
    {
       $focus->asterisk_outbound_c = 0;
    }
    if (isset($_REQUEST['asterisk_ext_c']))
    {
       $focus->asterisk_ext_c = $_REQUEST['asterisk_ext_c'];
    }
    else
    {
       $focus->asterisk_ext_c = '';
    }
    // </abcona>
